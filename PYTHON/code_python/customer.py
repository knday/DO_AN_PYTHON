...
#============NGHI==============
    ...
    def fetch_data(self):
        conn=mysql.connector.connect(host="localhost",username="root",password="Test@123", database="management")
        my_cursor=conn.cursor()
        my_cursor.execute("select . form customer") #27:37 3/6 dau cham
        rows=my_cursor.fetchall()
        if len(rows)!=0:
            self.Cust_Details_Table.delete(self.Cust_Details_Table.get_children()) #dau cham 27:37 3/6
            for i in rows:
    ...
    def get_cuersor(self,event=""):
        cusror_row=self.Cust_Details_Table.focus()
        content=self.Cust_Details_Table.item(cusror_row)
        row=content["values"]
    ...
    def mDelete(self):
        ...
        if mDelete>0:
            conn=mysql.connector.connect(host="localhost",username="root",password="Test@123", database="management")
            my_cursor=conn.cursor()
            query="delete form customer where Ref=%s"
    ...
    def search(self):
        ...
        my_cursor.execute("select . from customer where "+str(self.search_var.get())+"LIKE'%s"+str(self.txt_search.get())+"%'") #dau cham 54:50 3/6
        rows=my_cursor.fetchall()
        if len(rows)!=0:
            self.Cust_Details_Table.delete(self.Cust_Details_Table.get_children()) #dau chamn 59:56 3/6
            for i in rows:
                self.Cust_Details_Table.insert("",END,values=i)
            conn.commit()
        conn.close()
